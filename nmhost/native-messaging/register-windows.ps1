...
.DESCRIPTION
    ...
    Реального расширения браузера ещё нет (см. AGENTS.md) — allowed_origins
    в манифесте содержит плейсхолдер вместо настоящего id расширения;
    замените его через -ExtensionId, когда расширение появится.
...
.PARAMETER ExtensionId
    Id расширения Chrome, которому разрешено говорить с этим хостом
    (подставляется в allowed_origins манифеста). Плейсхолдер по умолчанию —
    реального расширения ещё нет.
