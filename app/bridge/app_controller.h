...
// actual vault logic lives in VaultService (Qt-free, in libs/core) тАФ this
...
// path under the platform's app-data directory (see defaultVaultPath()) тАФ
...
public:
    ...
    // configured тАФ checked WITHOUT unlocking (the unlock screen needs this
    ...
    // Security preferences тАФ persisted across runs via QSettings (see
    ...
    // shown тАФ true until dismissWelcome() is called once, ever, persisted
...
public slots:
    ...
    // Dispatches to createVault() or unlock() depending on vaultExists() тАФ
    ...
    // the default vault тАФ readable without unlocking. Empty if the vault
    ...
    // wherever an entry's folder is being set/cleared (setEntryFolder) тАФ
    ...
    // (the format QR codes decode to тАФ paste-in, no image import here; see
    ...
    // taken from the URI's issuer, falling back to its label) тАФ the "add a
    ...
    // Pure decode step тАФ the caller (LinkTotpDialog.qml) feeds the result
