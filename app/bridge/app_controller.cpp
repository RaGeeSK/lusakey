...
AppController::AppController(QObject* parent)
    ...
    // Enumerate available font families once at construction time so QML
    // can populate a picker without accessing QFontDatabase at all.
    fontFamilies_ = QFontDatabase::families(QFontDatabase::Any);
    ...
    // Always running (not gated on unlocked/lock state) тАФ tickTotpList()
...
QString AppController::defaultVaultPath() const {
    ...
    // real vault. No-op unless explicitly set тАФ see AGENTS.md.
...
void AppController::updateEntry(qulonglong entryId, const QString& title, const QString& username,
    ...
    try {
        ...
        // touches тАФ tags/folderId/totp тАФ so saving an edit doesn't silently
...
QString AppController::decodeTotpQrImage(const QUrl& fileUrl) {
    ...
    // Format_Grayscale8) тАФ copy row by row, not a single memcpy of the
...
void AppController::refreshTotpList() {
    // Deliberately ignores searchText_ тАФ the Authenticator Codes tab shows
